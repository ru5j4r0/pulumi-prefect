...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `prefect` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `ru5j4r0` with the `ORG` of your provider.
   - replace all instances of the `github.com/ru5j4r0/pulumi-prefect` repository with the `REPOSITORY` location
...
before:
  ...
  - make tfgen
builds:
- binary: pulumi-resource-prefect
  dir: provider
  goarch:
  - amd64
  - arm64
  ...
  ignore: []
  ldflags:
  - -X github.com/ru5j4r0/pulumi-prefect/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-prefect/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-prefect_darwin_amd64_v1
│   └── pulumi-resource-prefect
├── pulumi-prefect_darwin_arm64
│   └── pulumi-resource-prefect
├── pulumi-prefect_linux_amd64_v1
│   └── pulumi-resource-prefect
├── pulumi-prefect_linux_arm64
│   └── pulumi-resource-prefect
├── pulumi-prefect_windows_amd64_v1
│   └── pulumi-resource-prefect.exe
└── pulumi-prefect_windows_arm64
    └── pulumi-resource-prefect.exe
```
