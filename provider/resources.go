...
import (
	...
	prefect "github.com/prefecthq/terraform-provider-prefect/shim"

	"github.com/ru5j4r0/pulumi-prefect/provider/pkg/version"
)
...
const (
	...
	mainPkg = "prefect"
	// modules:
	mainMod = "index" // the prefect module
)

//go:embed cmd/pulumi-resource-prefect/bridge-metadata.json
var metadata []byte
...
func Provider() tfbridge.ProviderInfo {
	...
	prov := tfbridge.ProviderInfo{
		...
		//
		//    - In provider/cmd/pulumi-tfgen-prefect/main.go, replace the
		...
		//
		//    - In provider/cmd/pulumi-resource-prefect/main.go, replace the
		...
		//
		//      	tfbridge.Main(context.Background(), "prefect", prefect.Provider(),
		...
		// convention is to have a function at "github.com/iwahbe/terraform-provider-prefect/provider".New
		...
		//
		//nolint:lll
		P: pf.MuxShimWithPF(context.Background(), pf.ShimProvider(prefect.NewProvider()), prefect.NewProvider()),
		...
		Keywords:   []string{"prefect", "prefect-cloud", "category/cloud"},
		License:    "Apache-2.0",
		...
		Config:       map[string]*tfbridge.SchemaInfo{
			...
			//"account_id": {
			//	Type: tfbridge.MakeResource(mainPkg, mainMod, "AccountId"),
			//	Default: &tfbridge.DefaultInfo{
			//		EnvVars: []string{"PREFECT_CLOUD_ACCOUNT_ID"},
			//	},
			...
			//},
			//"api_key": {
			//	Type: tfbridge.MakeResource(mainPkg, mainMod, "APIKey"),
			//	Default: &tfbridge.DefaultInfo{
			//		EnvVars: []string{"PREFECT_API_KEY"},
			//	},
			//	MarkAsOptional: tfbridge.True(),
			//},
			//"endpoint": {
			//	Type: tfbridge.MakeResource(mainPkg, mainMod, "Endpoint"),
			//	Default: &tfbridge.DefaultInfo{
			//		EnvVars: []string{"PREFECT_API_URL"},
			//	},
			//	MarkAsOptional:        tfbridge.True(),
			//	ForceNew:              tfbridge.True(),
			//	ForcesProviderReplace: tfbridge.True(),
			//},
			//"workspace_id": {
			//	Type:                  tfbridge.MakeResource(mainPkg, mainMod, "WorkspaceId"),
			//	MarkAsOptional:        tfbridge.True(),
			//	ForceNew:              tfbridge.True(),
			//	ForcesProviderReplace: tfbridge.True(),
			//},
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: path.Join(
				"github.com/ru5j4r0/pulumi-prefect/sdk/",
	...
	prov.MustComputeTokens(tokens.SingleModule("prefect_", mainMod,
